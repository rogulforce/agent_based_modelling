...
class GifTool:
    ...
    def __init__(self, pic_dir: str = 'data/temp', gif_dir: str = 'data/gif', cmap=None, bounds=None):
        ...
        if not cmap:
            self.cmap = colors.ListedColormap(['white', 'gainsboro', 'silver', 'grey', 'dimgrey', 'black'])
            bounds = [-10, -0.5, 1.5, 2.5, 3.5, 4.5, 5.5]
        else:
            self.cmap = cmap
            bounds = bounds
    ...
    def visualize(self, data):
        ...
        img = plt.matshow(np.expand_dims(data, axis=0), interpolation='nearest', origin='lower',
